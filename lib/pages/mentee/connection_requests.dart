...
class _RequestPageState extends State<RequestPage> {
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator(color: Color(0xFF687EFF)));
          }

          if (snapshot.hasError) {
            print("Error: ${snapshot.error}");
            return Center(
              child: Text('Error fetching requests: ${snapshot.error}', style: TextStyle(color: Colors.red)),
            );
          }

          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return const Center(
              child: Text('No received requests.', style: TextStyle(color: Colors.white)),
            );
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return FutureBuilder<DocumentSnapshot>(
                future: FirebaseFirestore.instance.collection('userinfo').doc(fromUserId).get(),
                builder: (context, userSnapshot) {
                  ...
                  if (userSnapshot.hasError) {
                    return const ListTile(
                      title: Text("Error loading user info", style: TextStyle(color: Colors.white)),
                    );
                  }
  ...
  Future<void> acceptRequest(String requestId, String fromUserId) async {
    try {
      ...
      await FirebaseFirestore.instance.collection('connections').doc(requestId).set({
        'from': fromUserId,
